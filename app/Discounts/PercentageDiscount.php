<?php
    ...
    use App\Models\PercentageDiscount as PercentageDiscountAlias;
    ...
    class PercentageDiscount implements DiscountInterface
    {
        ...
        {
            return PercentageDiscountAlias::where('is_active', true)
                                          ->where('start', '<=', now())
                                          ->where('end', '>=', now())
                                          ->get();
        }

        public function calcCartItemDiscount(array $discountedProductIds, PercentageDiscountAlias $activeDiscount):float
        {
            $discount = 0;
            foreach ($this->cartItems as $cartItem)
            {
                if(in_array($cartItem,$discountedProductIds)) {
                    $productPrice = CartItem::first(['id' => $cartItem])->price;
                    $discount += ($productPrice * $activeDiscount->discount) / 100;
                }
            }
